...
func TestVerifyEd25519(t *testing.T) {
	for _, data := range ed25519TestData {
		...
		if err != nil {
			t.Fatalf("Unable to parse Ed25519 public key: %v", err)
		}
		...
		if data.valid {
			...
			if err == nil {
				t.Error("Invalid key passed validation")
			}
		}
	}
}
...
func TestSignEd25519(t *testing.T) {
	for _, data := range ed25519TestData {
		...
		if err != nil {
			t.Fatalf("Unable to parse Ed25519 private key: %v", err)
		}
		...
		if ssig == parts[2] && !data.valid {
			t.Errorf("Identical signatures found when data was invalid: %s", ssig)
		}
	}
}
